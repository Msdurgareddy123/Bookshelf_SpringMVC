...
public abstract class AbstractMongoDAO {
    ...
    public <T extends AbstractMongoEntity> Collection<T> getAllBooks(Class<T> clazz) {
    ...
    public <T extends AbstractMongoEntity> T getEntityByCId(String id, Class<T> clazz) {
        Query query = new Query(Criteria.where("cid").is(id));
        return getMongoOperations().findOne(query, clazz, clazz.getSimpleName());
    }
    public <T extends AbstractMongoEntity> T getEntityByPId(String id, Class<T> clazz) {
